    ...
    <View style={styles.container}>
      ...
      >
        {routes.map((route) => {
          ...
          const duration = t("metrics.minutes", {
            value: formatDuration(route.metrics.duration_s),
          });
            ...
            >
              ...
              {transitLeg?.transit ? (
                <View style={[styles.transitSummary, rtl && styles.rowReverse]}>
                  ...
                  <View style={styles.transitCopy}>
                    <Text
                      style={[styles.transitHeadsign, rtl && styles.rtl]}
                      numberOfLines={1}
                    >
                      {transitLeg.transit.headsign || transitLeg.to_name}
                    </Text>
                    <Text
                      style={[styles.transitTime, rtl && styles.rtl]}
                      numberOfLines={1}
                    >
                    ...
                    {transitLeg.transit.vehicle_rule_source?.includes(
                      "folded-vehicle",
                    ) ? (
                      <Text
                        style={[styles.vehicleRule, rtl && styles.rtl]}
                        numberOfLines={1}
                      >
              ...
              <View style={[styles.metricRow, rtl && styles.rowReverse]}>
                ...
                {route.metrics.traffic_signals != null ? (
                  <Metric
                    color={colors.mixed}
                    text={t("metrics.signals", {
                      value: route.metrics.traffic_signals,
                    })}
                  />
                ) : null}
                {route.metrics.signals_avoided ? (
                  <Metric
                    ...
                    text={t("metrics.signalsAvoided", {
                      value: route.metrics.signals_avoided,
                    })}
                  />
                ) : null}
                {transitLeg ? (
                  <Metric
                    ...
                    text={t("metrics.transfers", {
                      value: route.metrics.transfers,
                    })}
                  />
                ) : null}
              ...
              {fallback ? (
                <Text
                  style={[styles.note, rtl && styles.rtl]}
                  numberOfLines={2}
                >
                  {fallback}
                </Text>
              ...
              {transitLeg ? (
                <Pressable
                  ...
                  onPress={() =>
                    void Linking.openURL("https://transitous.org/sources/")
                  }
...
const styles = StyleSheet.create({
  ...
  etaGroup: { flexDirection: "row", gap: spacing.md, alignItems: "baseline" },
  duration: { fontSize: 23, fontWeight: "900", color: colors.primaryDark },
